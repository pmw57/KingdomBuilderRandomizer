...
const harvest = (function iife() {
    "use strict";
    const href = "https://boardgamegeek.com/boardgameexpansion/195267/kingdom-builder-harvest";
    const locations = "Bazaar/Water Mill, Mountain Station/Scout Cabin, Cathedral/Watch Tower, University/Pallisade";
    const boardsList = ["Bazaar", "Mountain Station", "Cathedral", "University"];
    const goalsList = ["Rangers", "Mayors", "Chainers", "Travellers", "Homesteaders", "Rovers"];
    function initHarvest(document, data) {
        function addHarvest(data) {
            if (!document.querySelector("#harvest")) {
                ...
                const checkbox = html.create("input", {type: "checkbox", id: "harvest", checked: "checked"});
                const link = html.create("a", {content: "Harvest", href, class: "harvest", title: locations});
                ul.appendChild(li);
